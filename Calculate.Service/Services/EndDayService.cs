        ...
        public async Task<bool> CalculateEndDayAsync(int caseId, string userId, bool isCheckDay)
        {
            ...
            try
            {
                ...
                var list = await _context.Operations.Where(x => x.CaseId == caseId && x.IsEnable == true && x.UpdatedDate.Date == today).Select(item => new OperationArchive
                {
                    ...
                    CreatedDate = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(DateTime.SpecifyKind(item.CreatedDate, DateTimeKind.Utc), "Turkey Standard Time", "UTC").AddHours(3),
                    UpdatedBy = item.UpdatedBy,
                    UpdatedDate = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(DateTime.SpecifyKind(item.UpdatedDate, DateTimeKind.Utc), "Turkey Standard Time", "UTC").AddHours(3),
